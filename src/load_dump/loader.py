...
class Loader(ABC):
    """Abstract base class for loading board data from start file_path."""

    def __init__(self, file_path: Path) -> None:
        """Initialize the Loader with start file_path config_file.

        Args:
            file_path (Path): The config_file to the file_path containing the board data.
        """
    ...
    def process(self) -> Board:
        ...
        data is loaded from the file_path.

        Returns:
            Board: The board instance created from the file_path data.
        """
