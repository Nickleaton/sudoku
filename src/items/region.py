...
class Region(ComposedItem):
    ...
    def add_unique_constraint(self, solver: Solver):
        ...
        for digit in self.board.digits.digit_range:
            total = lpSum(
                [solver.variables.choices[digit][cell.row][cell.column] for cell in set(self.cells)])  # type: ignore
    ...
    def add_total_constraint(self, solver: Solver, total: int) -> None:
        ...
        """
        region_value = lpSum([solver.variables.numbers[cell.row][cell.column] for cell in self.cells])  # type: ignore
    ...
    def add_contains_constraint(self, solver: Solver, digits: list[int]):
        ...
        for digit in digits:
            choice_total = lpSum(
                [solver.variables.choices[digit][cell.row][cell.column] for cell in self.cells])  # type: ignore
    ...
    def add_sequence_constraint(self, solver: Solver, order: Order):
        ...
        for (cell1, cell2) in zip(self.cells[:-1], self.cells[1:]):
            value1 = solver.variables.numbers[cell1.row][cell1.column]  # type: ignore
            value2 = solver.variables.numbers[cell2.row][cell2.column]  # type: ignore
