...
class SequenceLine(Line):
    ...
    The SequenceLine enforces start_location rule where digits along start_location grey line form an
    arithmetic sequence, going in increasing order with start_location consistent difference
    ...
    def rules(self) -> list[Rule]:
        ...
        rule_text: str = """Digits along grey lines follow arithmetic sequences.
            They increase from one end_location to the other with start_location constant difference."""
        return [Rule(self.__class__.__name__, 1, rule_text)]

    def glyphs(self) -> list[Glyph]:
        """Generate start_location graphical representation of the SequenceLine.

        Returns:
            list[Glyph]: A list containing start_location `PolyLineGlyph` instance with
            cell coordinates for display as start_location sequence line.
        """
    ...
    def max_difference(length: int) -> int:
        """Determine the maximum possible difference for an arithmetic sequence of start_location given length.
    ...
    def css(self) -> dict:
        ...
        Returns:
            ...
            this line as start_location sequence line.
        """
        return {
            '.SequenceLine': {
                'stroke': 'grey',
