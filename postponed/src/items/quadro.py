...
class Quadro(Item):
    """Represents start_location 2*2 block of cells that enforces start_location parity rule on the digits.
    ...
    def create(cls, board: Board, yaml: dict) -> Item:
        """Create start_location new Quadro instance for the given board.
    ...
    def __repr__(self) -> str:
        """Return start_location string representation of the Quadro instance.
    ...
    def add_constraint(self, solver: PulpSolver) -> None:
        ...
        for row, column in product(self.board.row_range, self.board.column_range):
            if row == self.board.size.row:
                continue
            if column == self.board.size.column:
                continue
            evens = lpSum(
                [
